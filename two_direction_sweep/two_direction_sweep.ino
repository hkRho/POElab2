...
void loop(){
    for (azimuth = 40; azimuth <= 140; azimuth += 2){ // increment the azimuth/yaw by 2 degrees each time
        yaw.write(azimuth);
        delay(15);
        
        for (elevation = 30; elevation <= 135; elevation += 2) {  // increment the elevation/pitch by 2 degrees each time
